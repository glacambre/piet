...
impl Lightness {
    fn diff_to (self, other: &Lightness) -> usize {
        use Lightness::*;
        match self {
            Light  => match *other { Light => 0, Normal => 1, Dark => 2 },
            Normal => match *other { Light => 2, Normal => 0, Dark => 1 },
            Dark   => match *other { Light => 1, Normal => 2, Dark => 0 },
        }
    }
}
...
impl Hue {
    fn diff_to (self, other: &Hue) -> usize {
        use Hue::*;
        match self {
            Red     => match *other { Red => 0, Yellow => 1, Green => 2, Cyan => 3, Blue => 4, Magenta => 5, White => 6, Black => 6},
            Yellow  => match *other { Red => 5, Yellow => 0, Green => 1, Cyan => 2, Blue => 3, Magenta => 4, White => 6, Black => 6},
            Green   => match *other { Red => 4, Yellow => 5, Green => 0, Cyan => 1, Blue => 2, Magenta => 3, White => 6, Black => 6},
            Cyan    => match *other { Red => 3, Yellow => 4, Green => 5, Cyan => 0, Blue => 1, Magenta => 2, White => 6, Black => 6},
            Blue    => match *other { Red => 2, Yellow => 3, Green => 4, Cyan => 5, Blue => 0, Magenta => 1, White => 6, Black => 6},
            Magenta => match *other { Red => 1, Yellow => 2, Green => 3, Cyan => 4, Blue => 5, Magenta => 0, White => 6, Black => 6},
            White   => 6,
            Black   => 6,
        }
    }
 ...
 * */
fn compare_farthest_codels <'a> (c1: &'a Codel, c2: &'a Codel, direction_pointer: Direction, codel_chooser: Direction) -> &'a Codel {
    use Direction::*;
    if c1 == c2 {
        c1
    } else {
        match direction_pointer {
            Right => if c1.x > c2.x { &c1 }
                        else if c1.x < c2.x { &c2 }
                        else {
                            match codel_chooser {
 ...
 * direction_pointer: The global direction pointer
 * codel_chooser: The global codel chooser
 ...
 * position according to the direction pointer and codel_chooser.
 * For white blocks the usize is 0 and the codel is the farthest white codel in a straight line in
 * direction_pointer's direction.
 *
 * This is basically a depth-first search implemented using a stack instead of recursion. */
fn get_farthest_codel (picture: &Vec<Vec<Codel>>, x:usize, y:usize, direction_pointer: Direction, codel_chooser: Direction) -> (usize, &Codel) {
    // White blocks have a different 
    if picture[y][x].color.hue == Hue::White {
        let (mut x, mut y) = (x as isize, y as isize);
        let (tmpx, tmpy) = direction_pointer.to_vector();
        while y >= 0 && y < picture.len() as isize
            && x >= 0 && x < picture[y as usize].len() as isize
    ...
    while codels_to_visit.len() > 0 {
        ...
        if tmpy + 1 < picture.len()
            && &picture[tmpy + 1][tmpx].color == &picture[y][x].color
        ...
        result = compare_farthest_codels(result, &picture[tmpy][tmpx], direction_pointer, codel_chooser);
    }
 ...
 * direction of the direction_pointer within the picture.
 * returns: A (bool, bool, &Codel) tuple. The first boolean is true if there is an available codel
 * in the direction of the direction pointer, false otherwise. The second bool indicates whether
 * the algorithm encountered white blocks between the current codel and the newly found codel. The
 * returned Codel is the new codel when the bool is true and the starting codel when the bool is
 * false.
 */
fn can_go_in_direction<'a> (picture: &'a Vec<Vec<Codel>>, current_codel: &'a Codel, direction_pointer: Direction) -> (bool, &'a Codel) {
    let (mut tmpx, mut tmpy) = (current_codel.x as isize, current_codel.y as isize);
    let vec = direction_pointer.to_vector();
    tmpx += vec.0;
    tmpy += vec.1;
...
fn get_picture (filename: std::string::String) -> Vec<Vec<Codel>> {
    ...
    let mut picture: Vec<Vec<Codel>> = vec![vec![Codel { color: PietColor { hue: Hue::White, lightness: Lightness::Normal }, x: 0, y: 0 }; pic_width]; pic_height];
    let mut i = 0;
...
fn main() {
    ...
    let mut direction_pointer = Direction::Right;
    let mut codel_chooser = Direction::Left;

    let mut current_codel = picture[0][0].clone();
    'main_loop: loop {
        if debug {
            println!("{:?}, {:?}, {:?}", current_codel, direction_pointer, codel_chooser);
            println!("{:?}", piet_stack);
        ...
        let mut block_size;
        'codel_choosing: loop {
            let result = get_farthest_codel(&picture, current_codel.x, current_codel.y, direction_pointer, codel_chooser);
            block_size = result.0;
            let farthest_codel = result.1;
            match (attempts % 2, can_go_in_direction(&picture, farthest_codel, direction_pointer)) {
                (_, (true, codel)) => { next_codel = codel; break 'codel_choosing; },
                (0, (false,  codel)) => { current_codel = codel.clone(); codel_chooser = codel_chooser.opposite(); },
                (1, (false,  codel)) => { current_codel = codel.clone(); direction_pointer = direction_pointer.rotate(); },
                _ => unreachable!(),
            }
            attempts += 1;
            if attempts >= 8 {
        ...
        match current_codel.diff_to(next_codel) {
            ...
            (0, 2) => { piet_stack.pop(); },
            (1, 0) => if let (Some(a), Some(b)) = (piet_stack.pop(), piet_stack.pop()) { piet_stack.push(b + a); },
            (1, 1) => if let (Some(a), Some(b)) = (piet_stack.pop(), piet_stack.pop()) { piet_stack.push(b - a); },
            (1, 2) => if let (Some(a), Some(b)) = (piet_stack.pop(), piet_stack.pop()) { piet_stack.push(b * a); },
            (2, 0) => if let (Some(a), Some(b)) = (piet_stack.pop(), piet_stack.pop()) { piet_stack.push(b / a); },
            (2, 1) => if let (Some(a), Some(b)) = (piet_stack.pop(), piet_stack.pop()) { piet_stack.push(b % a); },
            (2, 2) => if let Some(val) = piet_stack.pop() { piet_stack.push(if val == 0 { 1 } else { 0 }) },
            (3, 0) => if let (Some(a), Some(b)) = (piet_stack.pop(), piet_stack.pop()) { piet_stack.push(if b > a { 1 } else { 0 }) },
            (3, 1) => if let Some(a) = piet_stack.pop() {
                for _ in 0..a { direction_pointer = direction_pointer.rotate(); }
            },
            (3, 2) =>  if let Some(a) = piet_stack.pop() {
                for _ in 0..a { codel_chooser = codel_chooser.opposite(); }
            },
            (4, 0) => if let Some(a) = piet_stack.pop() {
                piet_stack.push(a);
                piet_stack.push(a);
            },
            (4, 1) => if let (Some(count), Some(depth)) = (piet_stack.pop(), piet_stack.pop()) {
                let mut before: Vec<_> = piet_stack.iter().take(piet_stack.len() - (depth as usize)).map(|e| *e).collect();
                if count >= 0 {
                    let mut rolled: Vec<_> = piet_stack.iter().skip(piet_stack.len() - (count as usize)).map(|e| *e).collect();
                    let mut after: Vec<_> = piet_stack.iter().skip(piet_stack.len() - (depth as usize)).take((depth - count) as usize).map(|e| *e).collect();
                    before.append(&mut rolled);
                    before.append(&mut after);
                } else {
                    let mut rolled: Vec<_> = piet_stack.iter().skip(before.len()).take(count.abs() as usize).map(|e| *e).collect(); 
                    let mut after: Vec<_> = piet_stack.iter().skip(before.len() + rolled.len()).map(|e| *e).collect();
                    before.append(&mut after);
            ...
            (4, 2) => {
                ...
                match std::io::stdin().read_line(&mut input) {
                    ...
                    Err(error) => if debug { panic!(error) },
                }
            },
            (5, 0) => { let stdin = std::io::stdin();
                    let mut chars = stdin.lock().chars();
                    match chars.nth(0) {
                        Some(Ok(char)) => if let Some(char) = char.to_digit(10) {
                                piet_stack.push(char as isize);
                            }
                        _ => (),
                    }
                },
            (5, 1) => if let Some(val) = piet_stack.pop() { print!("{}", val) },
            (5, 2) => if let Some(val) = piet_stack.pop() { print!("{}", std::char::from_u32(val as u32).unwrap()) },
            (6, _) => (),
            (a, b) => println!("Error: differences are ({},{}))", a, b),
        }
